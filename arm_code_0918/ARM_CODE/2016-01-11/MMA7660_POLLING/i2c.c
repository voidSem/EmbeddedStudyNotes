...
{   //预分频 高频 低频
    ...
    //通过nOpClock来确定第一分频跟第二分频
	if (!I2C_CalculatePrescaler(nPclk, nOpClock, &ePrescaler, &nPrescaler))                             //时钟源  参考频率  第一分频  第二分频
	{
...
bool I2C_RecvForGeneral(I2C_CHANNEL eCh, u8 ucSlvAddr, u8 aAddr[], unsigned int nNumOfAddr, u8 aData[], unsigned int nNumOfData)
{//发送主机所要读取从机寄存器的值的地址，然后读取从机寄存器的值
	bool bResult = TRUE;
	unsigned int i;
    //判断是钟是否无效
	if (g_aI2c[eCh].eError == I2C_INVALID_TX_CLOCK)
    ...
    //1.GPIO  prescale  filter  ACK
	I2C_InitMaster(eCh, g_aI2c[eCh].ePrescaler, g_aI2c[eCh].nPrescaler);	// Init I2C

	//
	// I2C Phase - START
	// //先设置成主机发送模式  发送所需要读的从机寄存器的地址的值
	I2C_SetMode(eCh, MASTER_TX_MODE);										// Set Mode  写从机地址
	I2C_WriteAddress(eCh, ucSlvAddr);										// Write Slave Address
	I2C_GenerateSignal(eCh, START_CONDITION);								// Send START Signal    //发送开始信号  从机地址就自动发送出去了
	if (I2C_WaitForXferAck(eCh, g_aI2c[eCh].nTimeOut) == FALSE)
	{//等待I2C应答
		g_aI2c[eCh].eError = I2C_TIMEOUT_SLAVE_ADDRESS;
		goto I2C_Stop;
	...
	for (i = 0; i < nNumOfAddr; i++)
	{  //发送从机寄存器地址
		I2C_WriteData(eCh, aAddr[i]);  //1.写数据
		I2C_ClearIntStatus(eCh);       //2.清中断
		if (I2C_WaitForXferAck(eCh, g_aI2c[eCh].nTimeOut) == FALSE)
		{//3.等待应答
			g_aI2c[eCh].eError = I2C_TIMEOUT_WRITE_ADDRESS;
			goto I2C_Stop;
	...
	for (i = 0; i < nNumOfData; i++)
	{
		if (i < nNumOfData-1)
		{
			I2C_SetAckGeneration(eCh, TRUE);  //1.应答
			I2C_ClearIntStatus(eCh);          //2.清中断
			if (I2C_WaitForXferAck(eCh, g_aI2c[eCh].nTimeOut) == FALSE)
			{//3.等待应答
				g_aI2c[eCh].eError = I2C_TIMEOUT_SLAVE_ADDRESS;
		...
		{
			I2C_SetAckGeneration(eCh, FALSE);  //最后一个要发送停止信号就不发达应答信号了
			I2C_ClearIntStatus(eCh);
...
I2C_Stop:
	...
	I2C_SetMode(eCh, DISABLE_I2C);											// Deactivate I2C //设置模式为关闭模式
...
{//I2C等待总线不忙  Ready   给nTimeOut 0 就一直等待
	while (--nTimeOut)
	{
...
{   //1.关掉I2C使能
    ...
    //5.使能中断
	I2C_EnableInt(eCh);
	HW_REG32(g_aI2c[eCh].uBase, I2C_STAT) |= (0x1 << 4);			// Enable I2C   //6.使能I2C
}
