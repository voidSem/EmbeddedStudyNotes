...
int _start(void)
{
    ...
    *(unsigned int *)0x62000018 = (unsigned int)Fiq;
    ...
    unsigned int PicCopyAddr= 0x65000000 ; 
...
//    SDMMC_ReadBlocks(0x3A98, 0x1838 , (unsigned int *)PicCopyAddr);
    SDMMC_ReadBlocks(0x4000, 0x4000 , (unsigned int *)PicCopyAddr);
...
//                *(((unsigned int *)PicAddress) + row *800 + col)  = (unsigned int)RGB(0,0,255);
                *(((unsigned int *)PicAddress) + row *800 + col)  = ((unsigned int*)PicCopyAddr)[row*800+col+flag*(800*480)];
            }
        }
        __udelay(1000*1000);
    }
