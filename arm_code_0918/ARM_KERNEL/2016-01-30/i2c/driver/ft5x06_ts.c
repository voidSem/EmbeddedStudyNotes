...
//board info
static struct i2c_board_info   xxxinfo = {
    I2C_BOARD_INFO("ft5x0x_ts" , (0x70 >> 1)),
    .platform_data = &ft5x0x_pdata,
};

static struct i2c_adapter  myadapter = {
    .owner = THIS_MODULE,
    .name  = "myadapter",
}; 
/*---------------------------------------------------------
...
{
    //myadapter = i2c_get_adapter(1);
    i2c_add_adapter(&myadapter);
    i2c_new_device(&myadapter , &xxxinfo);
...
{
  //i2c_put_adapter(myadapter);
    i2c_del_adapter(&myadapter);
...
MODULE_AUTHOR("CJY");
MODULE_DESCRIPTION("uplooking ft5x0x TouchScreen driver");
MODULE_LICENSE("GPL");
